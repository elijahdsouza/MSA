(function () {
    ...
    ui.Pages.define("/pages/groupedItems/groupedItems.html", {
        ...
        navigateToGroup: function (key) {
            nav.navigate("/pages/CalenderPage/CalenderPage.html", { groupKey: key });
        },
        ...
        _itemInvoked: function (args) {
            if (appView.value === appViewState.snapped) {
                ...
                var grp = Data.items.getAt(args.detail.itemIndex);
                //var grp1 = FinancialCalculatorData.groups.items.getAt(args.detail.itemIndex);
                // If the page is not snapped, the user invoked an item.
                if (grp.title == "Calender") {
                    var item = Data.items.getAt(args.detail.itemIndex);
                    nav.navigate("/pages/CalenderPage/CalenderPage.html", { item: Data.getItemReference(item) });

                }
                else if (grp.title == "Over due") {
                    var item = Data.items.getAt(args.detail.itemIndex);
                    nav.navigate("/pages/OverDue/OverDue.html", { item: Data.getItemReference(item) });
                }
                else if (grp.title == "Remainder") {
                    var item = Data.items.getAt(args.detail.itemIndex);
                    nav.navigate("/pages/AddTask/AddTask.html", { item: Data.getItemReference(item) });
                }
            }
        }
    });
    function sendTileLocalImageNotification() {
        ...
        tileTextAttributes[0].innerText = "Notes";
